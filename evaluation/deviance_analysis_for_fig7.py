#!/usr/bin/env python3
"""
Figure 7-style deviance-explained analysis for GLM-Missense-rescued membership.

Goal
----
Test whether the four interpretable Figure 6 covariates fully explain which variants are
preferentially recovered by GLM-Missense.

Outcome:
    GLM-Missense_correct_le2

Predictors:
    log10(gnomAD4.1_joint_AF + pseudocount)
    lof.oe_ci.upper
    spliceai_DS_max
    is_radical_aa_change

Main metric:
    Cross-validated Bernoulli deviance explained:

        D^2 = 1 - logloss(model) / logloss(null)

    where the null model predicts the training-fold prevalence of GLM-Missense rescue.

Main plot
---------
The main Figure 7 plot intentionally shows only:

    1. Total D^2 explained by all four Figure 6 covariates
    2. Not explained by these covariates = 100 - total D^2

This avoids the misleading stacked-feature display that can occur when one feature has a
negative cross-validated Shapley contribution. Signed Shapley feature contributions are
still computed and saved in the TSV output, but are not stacked in the main plot.

Important
---------
Do NOT use class_weight='balanced' for the main D^2 analysis. Class weighting can be useful
for AUROC/ranking with imbalanced outcomes, but it changes the fitted class prior and can
make predicted probabilities poorly calibrated for log-loss / deviance explained. This
script uses unweighted logistic regression by default.

Outputs
-------
<out-prefix>.subset_d2.tsv
<out-prefix>.shapley_decomposition.tsv
<out-prefix>.full_model_summary.tsv
<out-prefix>.figure7_deviance_explained.pdf
<out-prefix>.figure7_deviance_explained.png

Optional diagnostic:
<out-prefix>.signed_shapley_contributions.pdf/png
    only if --plot-signed-shapley is provided.

PDF/editability
---------------
Matplotlib is configured with pdf.fonttype=42 and ps.fonttype=42 so text remains editable
as text in Adobe Illustrator in most workflows.

Example
-------
python decompose_glm_rescue_d2_by_feature_v3.py \
  --input GLM-Missense_merged_format.tsv \
  --out-prefix glm_rescue_d2_decomposition_v3 \
  --folds 5
"""
