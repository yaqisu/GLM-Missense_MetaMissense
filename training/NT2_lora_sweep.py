...
Total: 58 experiments distributed across 4 GPUs.
  - LoRA jobs: 1 GPU each → 4 concurrent jobs
  - Full fine-tune jobs: 2 GPUs each (DataParallel) → 2 concurrent jobs
"""
...
def build_experiment_configs():
    """Build all 22 experiment configs programmatically."""
    configs = []
    exp_id = 1

    # ------------------------------------------------------------------
    # Part 1: 18 LoRA sweep experiments
    # 2 embeddings × 3 classifiers × 3 LoRA ranks
    # ------------------------------------------------------------------
    embedding_options = ["mean_pool", "variant_position"]
    classifier_options = ["mlp", "transformer", "cnn"]
    lora_ranks = [8, 16, 32]
    lora_lrs = [1e-5, 3e-5, 5e-5]

    for emb in embedding_options:
        for clf in classifier_options:
            for rank in lora_ranks:
                for lr in lora_lrs:
                    # MLP takes a single pooled vector → use "mean_pool" / "variant_position"
                    # CNN / Transformer take full sequence → use "full-mean_pool" / "full-variant_position"
                    if clf == "mlp":
                        emb_strategy = emb
                    else:
                        emb_strategy = f"full-{emb}"
                    ...
                    configs.append({
                        ...
                        "batch_size": 4,
                        "num_steps": 4000,
                        "gradient_accumulation_steps": 1,  # no accumulation for 5k dataset
                        "learning_rate": lr,
                    })
                    exp_id += 1
    ...
    for lr in full_ft_lrs:
        configs.append({
            ...
            "batch_size": 8,
            "num_steps": 40000,
            "gradient_accumulation_steps": 1,  # no accumulation, batch_size=8
            "learning_rate": lr,
        })
        exp_id += 1
...
def run_all_experiments(train_path_sweep, val_path_sweep,
                        train_path_ft, val_path_ft,
                        output_dir, available_gpus=[0, 1, 2, 3], k=6):
    """Run all experiments with smart GPU scheduling.

    - LoRA experiments (sweep): use train_path_sweep / val_path_sweep (e.g. 5k samples)
    - Full fine-tune experiments (ft): use train_path_ft / val_path_ft (e.g. 44k samples)
    - LoRA: 1 GPU each → run len(available_gpus) in parallel
    ...
    # Separate LoRA vs full fine-tune experiments
    lora_configs = [c for c in EXPERIMENT_CONFIGS if c['fine_tuning'] == 'lora']
    ...
    logger.info(f"Total experiments: {len(EXPERIMENT_CONFIGS)} "
                f"({len(lora_configs)} LoRA + {len(unfreeze_configs)} full fine-tune)")
    logger.info(f"Available GPUs: {available_gpus}")
    logger.info(f"Sweep data: {train_path_sweep} / {val_path_sweep}")
    logger.info(f"Full FT data: {train_path_ft} / {val_path_ft}")

    all_summaries = []

    # ---- Run LoRA experiments: 1 GPU each, N_GPU parallel ----
    logger.info("=" * 80)
    logger.info(f"Running {len(lora_configs)} LoRA experiments ({len(available_gpus)} at a time)")
    logger.info("=" * 80)
    ...
    for i, cfg in enumerate(lora_configs):
        ...
        lora_args.append((cfg, train_path_sweep, val_path_sweep,
                         str(output_dir), gpu_id, hyperparams))

    num_parallel = len(available_gpus)
    for i in range(0, len(lora_args), num_parallel):
        batch = lora_args[i:i + num_parallel]
        logger.info(f"LoRA batch {i // num_parallel + 1}: "
...
def main():
    parser = argparse.ArgumentParser(
        description='Reviewer Response Sweep: LoRA grid + full fine-tune LR sweep for NT2'
    )
    parser.add_argument('--train_path_sweep', type=str, required=True,
                        help='Path to training data for LoRA sweep (TSV, e.g. 5k samples)')
    parser.add_argument('--val_path_sweep', type=str, required=True,
                        help='Path to validation data for LoRA sweep (TSV)')
    parser.add_argument('--train_path_ft', type=str, required=True,
                        help='Path to training data for full fine-tune (TSV, e.g. 44k samples)')
    parser.add_argument('--val_path_ft', type=str, required=True,
    ...
    summary_df = run_all_experiments(
        train_path_sweep=args.train_path_sweep,
        val_path_sweep=args.val_path_sweep,
        train_path_ft=args.train_path_ft,
        val_path_ft=args.val_path_ft,
